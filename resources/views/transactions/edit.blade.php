...
@section('title', 'تعديل المعاملة - الدفتر الرقمي')
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="page-subtitle">تعديل تفاصيل المعاملة المالية رقم #{{ $transaction->id }}</p>
            </div>
            ...
            <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('customer_supplier_id') is-invalid @enderror" id="customer_supplier_id" name="customer_supplier_id" >
                                ...
                                @foreach($customersSuppliers as $cs)
                                    <option value="{{ $cs->id }}" {{ old('customer_supplier_id', $transaction->customer_supplier_id) == $cs->id ? 'selected' : '' }}>
                                        {{ $cs->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" >
                                ...
                                <option value="credit" {{ old('type', $transaction->type) == 'credit' ? 'selected' : '' }}>دائن (مقبوضات)</option>
                                <option value="debit" {{ old('type', $transaction->type) == 'debit' ? 'selected' : '' }}>مدين (مدفوعات)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount"
                                       step="0.01"  placeholder="0.00" value="{{ old('amount', $transaction->amount) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                              rows="3" placeholder="وصف المعاملة (اختياري)">{{ old('description', $transaction->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
